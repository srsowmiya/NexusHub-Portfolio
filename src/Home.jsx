...
const TEAM = [
  {
    name: "Hari", role: "Founder & CEO", initials: "H", photo: "/images/Hari.png",
    desc: "Visionary leader and ServiceNow strategist driving the mission and growth of Nexus Hub.",
    experience: "8+ Years", location: "Coimbatore, India",
  ...
  {
    name: "Gokul", role: "Co-Founder & CTO", initials: "G", photo: "/images/Gokul.jpeg",
    desc: "Technical expert across ServiceNow modules, platform architecture, and enterprise integrations.",
    experience: "7+ Years", location: "Coimbatore, India",
  ...
  {
    name: "Priya Raj", role: "Head of Operations", initials: "PR", photo: null,
    desc: "Ensures seamless project delivery and exceptional client experience at every touchpoint.",
    experience: "6+ Years", location: "Coimbatore, India",
...
const ModuleDropdown = ({ selected, setSelected }) => {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(!open)}
        ...
        <div style={{flex:1, display:"flex", flexWrap:"wrap", gap:"5px", minHeight:"22px"}}>
          {selected.length === 0 ? (
            ...
            selected.map(s => (
              <span key={s.code} style={{display:"inline-flex", alignItems:"center", gap:"5px", background:"#2C1810", color:"#fff", fontSize:"11px", fontFamily:"monospace", fontWeight:"700", padding:"3px 8px", borderRadius:"6px", letterSpacing:".03em"}}>
                {s.code}
                <span onClick={(e) => { e.stopPropagation(); toggle(s); }} style={{cursor:"pointer", opacity:0.5, fontSize:"12px", lineHeight:1, marginLeft:"2px"}}>×</span>
              </span>
            ))
          )}
        </div>
        <div style={{display:"flex", alignItems:"center", gap:"8px", flexShrink:0, marginLeft:"8px"}}>
          {selected.length > 0 && (
            <span style={{background:"rgba(0,0,0,0.15)", color:"#555", fontSize:"10px", fontWeight:"700", padding:"2px 7px", borderRadius:"999px"}}>{selected.length}</span>
          )}
      ...
      {open && (
        <div style={{position:"absolute", top:"calc(100% + 6px)", left:0, right:0, zIndex:50, background:"#1a1a1a", border:"1.5px solid #333", borderRadius:"12px", overflow:"hidden", boxShadow:"0 16px 48px rgba(0,0,0,0.4)", maxHeight:"320px", overflowY:"auto"}}>
          <div style={{padding:"8px 14px", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", gap:"10px"}}>
            <button type="button" onClick={() => setSelected([...SERVICES])} style={{background:"none", border:"none", color:"rgba(255,255,255,0.4)", fontSize:"11px", cursor:"pointer", fontFamily:"'Outfit',sans-serif", padding:0}}>Select all</button>
            <span style={{color:"rgba(255,255,255,0.15)"}}>·</span>
            <button type="button" onClick={() => setSelected([])} style={{background:"none", border:"none", color:"rgba(255,255,255,0.4)", fontSize:"11px", cursor:"pointer", fontFamily:"'Outfit',sans-serif", padding:0}}>Clear</button>
          </div>
          {SERVICES.map((s) => {
            const checked = isSelected(s.code);
            return (
              <button key={s.code} type="button" onClick={() => toggle(s)}
                style={{display:"flex", alignItems:"center", gap:"12px", width:"100%", padding:"11px 16px", background: checked ? "rgba(255,255,255,0.05)" : "transparent", border:"none", cursor:"pointer", textAlign:"left", borderBottom:"1px solid rgba(255,255,255,0.04)", transition:"background .15s"}}
                onMouseEnter={e => { if (!checked) e.currentTarget.style.background="rgba(255,255,255,0.03)"; }}
                onMouseLeave={e => { if (!checked) e.currentTarget.style.background="transparent"; }}>
                <div style={{width:"16px", height:"16px", borderRadius:"4px", flexShrink:0, border: checked ? "none" : "1.5px solid rgba(255,255,255,0.2)", background: checked ? "#fff" : "transparent", display:"flex", alignItems:"center", justifyContent:"center", transition:"all .15s"}}>
                  {checked && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#111" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
                </div>
                <span style={{padding:"2px 8px", borderRadius:"6px", background: checked ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.07)", color: checked ? "#fff" : "rgba(255,255,255,0.6)", fontSize:"11px", fontFamily:"monospace", fontWeight:"700", flexShrink:0, letterSpacing:".03em"}}>{s.code}</span>
...
export default function App() {
  ...
  const handleSendMessage = async () => {
    ...
    const modulesList = formModules.length > 0
      ? formModules.map(m => `${m.code} — ${m.name}`).join(", ")
      : "None selected";
    ...
    <div className="overflow-x-hidden">
      <style>{`
        ...
        *, body       { font-family:'Outfit',sans-serif; }
        ...
        .btn-dark  { background:#2C1810; color:#fff !important; padding:13px 30px; border-radius:8px; font-weight:600; font-size:14px; border:none; cursor:pointer; transition:background .2s,transform .2s; display:inline-block; font-family:'Outfit',sans-serif; }
        .btn-dark:hover { background:#3E2723; transform:translateY(-1px); }
        .btn-ghost-dark { background:transparent; color:#2C1810 !important; padding:13px 30px; border-radius:8px; font-weight:500; font-size:14px; border:1.5px solid #d0d0d0; cursor:pointer; transition:border-color .2s; display:inline-block; }
        .btn-ghost-dark:hover { border-color:#2C1810; }
        .btn-light { background:#FF6B35; color:#fff !important; padding:13px 30px; border-radius:8px; font-weight:600; font-size:14px; border:none; cursor:pointer; transition:opacity .2s,transform .2s; display:inline-block; }
        .btn-light:hover { background:#E55A25; transform:translateY(-1px); }
        .btn-ghost-light { background:transparent; color:#fff !important; padding:13px 30px; border-radius:8px; font-weight:500; font-size:14px; border:1.5px solid rgba(255,255,255,.3); cursor:pointer; transition:border-color .2s; display:inline-block; }
        ...
        .member-panel { background:#2C1810; border-radius:20px; overflow:hidden; }
        ...
        @keyframes floatSN { 0%,100%{transform:rotate(-8deg) translateY(0px)} 50%{transform:rotate(-8deg) translateY(-14px)} }
      ...
      <header className={`relative left-0 right-0 z-50 nav-wrap transition-all duration-300 ${scrolled ? "shadow-sm" : ""}`}>
        <div className="flex items-center justify-between h-28 md:h-40 w-full" style={{paddingLeft:"0px", paddingRight:"40px"}}>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {NAV.map(l => <button key={l} onClick={() => go(l)} className="nav-link font-body">{l}</button>)}
          </nav>
          <div className="hidden md:flex items-center gap-3">
            <div className="hero-tag" style={{gap:"8px"}}><span className="live-dot" />Open to work</div>
            <button onClick={() => go("Contact")} className="btn-dark" style={{background:"#FF6B35"}}
              onMouseEnter={e => e.target.style.background="#E55A25"}
              onMouseLeave={e => e.target.style.background="#FF6B35"}>Let's Talk →</button>
          </div>
        ...
        {mobileOpen && (
          <div className="md:hidden bg-[#2C1810] border-t border-[#1a0f0a] px-6 py-6 flex flex-col gap-5">
            ...
            <button onClick={() => go("Contact")} className="btn-dark text-center" style={{background:"#FF6B35"}}>Let's Talk →</button>
          </div>
        )}
      </header>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 w-full relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-0">
            <div className="flex-1 lg:pr-10 pb-16 lg:pb-0">
              <Reveal delay={0.05}><div className="hero-tag mb-6">Leading ServiceNow Firm</div></Reveal>
              <Reveal delay={0.15}>
                <h1 className="font-bebas text-[clamp(3.5rem,9vw,8rem)] leading-none tracking-wide text-black mb-2">Where Connections</h1>
                <h1 className="font-display italic text-[clamp(2.5rem,6vw,5.5rem)] leading-none text-black mb-5">Power Solutions</h1>
              </Reveal>
              <Reveal delay={0.25}>
            ...
            <div className="flex-1 flex items-center justify-end w-full" style={{minHeight:"520px", position:"relative"}}>
              <lottie-player src="https://assets2.lottiefiles.com/packages/lf20_iorpbol0.json" background="transparent" speed="1"
                style={{width:"110%", height:"600px", marginRight:"-40px"}} loop autoplay />
              <div style={{position:"absolute", bottom:"22%", left:"12%", animation:"floatSN 4s ease-in-out infinite", zIndex:10, pointerEvents:"none", transform:"rotate(-8deg)"}}>
                <div style={{position:"relative", width:"58px", height:"58px"}}>
                  <div style={{position:"absolute", top:"6px", left:"6px", width:"58px", height:"58px", background:"#FF6B35", borderRadius:"13px", transform:"skewY(-4deg)"}} />
                  <div style={{position:"absolute", top:"3px", left:"3px", width:"58px", height:"58px", background:"#FF7F4D", borderRadius:"13px", transform:"skewY(-2deg)"}} />
                  <div style={{position:"absolute", top:"0", left:"0", width:"58px", height:"58px", background:"#FF8C5A", borderRadius:"13px", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:"1px"}}>
      ...
      <section id="about" className="sec-black py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-10 grid md:grid-cols-2 gap-16 md:gap-24 items-start">
          <div>
            ...
            <Reveal delay={0.2}>
              <p className="font-body text-white/45 leading-relaxed mb-5">Founded in 2026 in India by a passionate team of ServiceNow professionals. We specialize exclusively in the ServiceNow ecosystem — making us one of the most focused and capable partners in the region.</p>
              <p className="font-body text-white/30 leading-relaxed mb-10">Young, ambitious, and global in vision — built on the belief that the right implementation can unlock unlimited potential for any organization.</p>
            ...
            {[
              ...
              <Reveal key={item.num} delay={i * 0.1}>
                <div className="about-accordion" onClick={() => setActiveIdx(activeIdx === i ? null : i)}>
                  ...
                  {activeIdx === i && <p className="font-body text-white/40 text-sm leading-relaxed mt-4 pt-4 border-t border-white/8">{item.body}</p>}
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section id="services" className="sec-hero py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            ...
            <Reveal delay={0.15}><p className="font-body text-black/40 max-w-xs text-sm leading-relaxed">End-to-end implementation across all major ServiceNow modules</p></Reveal>
          </div>
          <div className="bg-[#d4d4d4] rounded-2xl border border-[#b0b0b0] overflow-hidden">
            {SERVICES.map((s, i) => (
              <Reveal key={s.code} delay={i * 0.03}>
                <div className="flex items-center justify-between px-6 py-5 border-b border-[#b8b8b8] last:border-0 hover:bg-[#bebebe] transition-colors cursor-pointer">
            ...
            {WHY.map((w, i) => (
              <Reveal key={w.t} delay={i * 0.08}>
                <div className="h-full p-8 rounded-2xl border border-white/8 bg-white/[0.02] hover:border-[#FF6B35] hover:bg-white/[0.05] transition-all duration-300 flex flex-col">
                  <div style={{fontSize:"11px", fontFamily:"monospace", color:"#FF6B35", fontWeight:"700", letterSpacing:".1em", marginBottom:"16px"}}>{String(i+1).padStart(2,"0")}</div>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-stretch">
            <div className={`flex flex-col gap-4 transition-all duration-500 ${selectedMember ? "lg:w-[340px] shrink-0" : "w-full grid grid-cols-1 md:grid-cols-3"}`}
              ...
              {TEAM.map((m, i) => (
                <Reveal key={m.name} delay={i * 0.1}>
                  <div onClick={() => handleMemberClick(m)}
                    className={`team-card bg-[#d4d4d4] border border-[#b0b0b0] rounded-2xl p-7 h-full ${selectedMember?.name === m.name ? "active" : ""}`}>
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-14 h-14 rounded-full border-2 border-black flex items-center justify-center font-display font-bold text-black text-base bg-[#c8c8c8] shrink-0">{m.initials}</div>
                      <div>
            ...
            {selectedMember && (
              <div className="member-panel entered flex-1 min-w-0 flex flex-col" style={{background:"#2C1810", borderRadius:"16px", overflow:"hidden"}}>
                <div style={{padding:"22px 22px 18px", borderBottom:"1px solid rgba(255,255,255,0.07)", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0}}>
                  <div>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setSelectedMember(null); }}
                    style={{background:"rgba(255,107,53,0.15)", border:"1px solid rgba(255,107,53,0.3)", borderRadius:"8px", width:"32px", height:"32px", display:"flex", alignItems:"center", justifyContent:"center", color:"#FF6B35", fontSize:"16px", cursor:"pointer", flexShrink:0, lineHeight:1}}>×</button>
                </div>
                  ...
                  <div style={{flex:1, minWidth:0, padding:"22px 24px", overflowY:"auto", display:"flex", flexDirection:"column", gap:"20px"}}>
                    ...
                    <div>
                      ...
                      <div style={{display:"flex", flexWrap:"wrap", gap:"6px"}}>
                        {selectedMember.expertise.map(e => <span key={e} className="expertise-tag" style={{fontSize:"12px", padding:"5px 12px"}}>{e}</span>)}
                      </div>
                    </div>
                    <div>
                      ...
                      <div style={{display:"flex", flexWrap:"wrap", gap:"6px"}}>
                        {selectedMember.modules.map(mod => <span key={mod} className="module-chip" style={{fontSize:"12px"}}>{mod}</span>)}
                      </div>
                    </div>
                  </div>
                  <div style={{width:"320px", flexShrink:0, overflow:"hidden", borderLeft:"1px solid rgba(255,255,255,0.06)"}}>
                    {selectedMember.photo
                      ? <img src={selectedMember.photo} alt={selectedMember.name} style={{width:"100%", height:"100%", objectFit:"cover", objectPosition:"center 15%", display:"block"}} />
          ...
          <Reveal>
            <div className="relative rounded-3xl border border-white/10 bg-white/[0.03] p-16 md:p-24 text-center overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="font-body text-white/35 max-w-md mx-auto mb-10 text-sm leading-relaxed">Let's talk about how Nexus Hub can unlock the full potential of ServiceNow for your organization.</p>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 grid md:grid-cols-2 gap-16 md:gap-24 items-start">
          <div>
            ...
            <Reveal delay={0.2}>
              ...
              <div className="mt-10">
                <a href="https://www.linkedin.com/company/nexus-hub-global-solutions/" target="_blank" rel="noopener noreferrer"
            ...
            <div className="bg-[#d4d4d4] border border-[#b0b0b0] rounded-2xl p-8">
              ...
              {formStatus === "success" ? (
                <div style={{display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"24px", padding:"60px 24px", textAlign:"center", minHeight:"500px", position:"relative", overflow:"hidden"}}>
                  {[...Array(15)].map((_, i) => (
                    <div key={i} style={{position:"absolute", width:"10px", height:"10px", background:["#FF6B35","#2C1810","#FF6B35","#C8C8C8"][i%4], borderRadius:"50%", animation:`confetti-fall ${2+Math.random()}s ease-in forwards`, left:`${Math.random()*100}%`, top:"-10px", opacity:0.8}} />
                  ))}
                  <div style={{position:"relative", zIndex:10, animation:"scale-in 0.6s cubic-bezier(.22,1,.36,1)"}}>
                  ...
                  <div style={{position:"relative", zIndex:10}}>
                    ...
                    <p className="font-body text-black/60 text-base" style={{marginBottom:"24px"}}>Thank you! We'll get back to you within one business day.</p>
                    <button onClick={() => setFormStatus("idle")} className="btn-ghost-dark" style={{padding:"12px 28px", fontSize:"14px", fontWeight:"600"}}>Send Another Message →</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input className="ci" placeholder="Arjun" name="firstName" value={formData.firstName} onChange={handleFormChange}
                    ...
                    <div>
                      ...
                      <input className="ci" placeholder="Kumar" name="lastName" value={formData.lastName} onChange={handleFormChange} />
                    </div>
                  </div>
                  <div>
                    <label className="font-body text-black/40 text-xs mb-2 block">Email Address *</label>
                    <input className="ci" placeholder="arjun@company.com" name="email" value={formData.email} onChange={handleFormChange}
                  ...
                  <div>
                    ...
                    <textarea className="ci resize-none" rows={4} placeholder="Tell us about your project..." name="message" value={formData.message} onChange={handleFormChange} />
                  </div>

                  {formStatus === "error_validation" && <p style={{color:"#c00", fontSize:"12px", fontFamily:"'Outfit',sans-serif"}}>Please fill in your first name and email.</p>}
                  {formStatus === "error" && <p style={{color:"#c00", fontSize:"12px", fontFamily:"'Outfit',sans-serif"}}>Something went wrong. Please try again.</p>}

                  <button onClick={handleSendMessage} disabled={formStatus === "sending"} className="btn-dark w-full text-center font-body"
                    style={{opacity: formStatus === "sending" ? 0.6 : 1, cursor: formStatus === "sending" ? "not-allowed" : "pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", background:"#FF6B35"}}>
                    {formStatus === "sending" ? (
                      <>
      ...
      <footer className="footer-wrap py-10 px-6 md:px-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <a href="https://www.linkedin.com/company/nexus-hub-global-solutions/" target="_blank" rel="noopener noreferrer"
            style={{display:"inline-flex", alignItems:"center", gap:"8px", padding:"8px 16px", borderRadius:"8px", border:"1px solid #FF6B35", color:"#FF6B35", fontSize:"12px", fontFamily:"'Outfit',sans-serif", fontWeight:"500", textDecoration:"none", transition:"all .2s"}}
